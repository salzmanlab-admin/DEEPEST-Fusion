...
def parseChrFeatures(chrSeqRecord):
    ...
    for ftr in chrSeqRecord.features:  # this is 1 entire gene
        try:
            ...
            for sftr in ftr.sub_features:
                ...
                if sftr.strand == 1:
                    exonsPos[(int(sftr.location.start), int(sftr.location.end), sftr.qualifiers["gene_name"][0]
)] = sftr
                else:
                    exonsNeg[(int(sftr.location.start), int(sftr.location.end), sftr.qualifiers["gene_name"][0]
)] = sftr
            ...
            if sftr:
                ...
                if ftr.strand == 1:
                    exonsPos[(int(ftr.location.start), int(ftr.location.end), ftr.qualifiers["gene_name"][0])] = ftr
                else:
    ...
    if len(exonsPos) > 0:
        chrExonsByDirection.append(
            (chrSeqRecord.id, 1, exonsPos))  # chrSeqRecord.id is chr1 for example, 1 indicates pos strand
    if len(exonsNeg) > 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--outDir', help='directory to output files, will be created if it does not exist',
                        default='output')
    ...
    parser.add_argument('-n2', '--name2', help='name of field in gtf to use for gene names if n1 does not exist',
                        default='gene_id')
    parser.add_argument('-e', '--exonOutDir',
                        help='directory to output exon pickle files, will be created within outDir if it does not exist',
                        default='exons')
    parser.add_argument('-g', '--geneOutDir',
                        help='directory to output gene pickle files, will be created within outDir if it does not exist',
                        default='genes')
    parser.add_argument('-r', '--recOutDir',
                        help='directory to output seq record pickle files, will be created within outDir if it does not exist',
                        default='records')
